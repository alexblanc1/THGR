...
\usepackage{amssymb} % pour les produits semis directs
...
\textbf{Direction artistique :}
    \begin{enumerate}[label=\roman*)]
        ...
        \item Posons $K = GL_n(\Z / p\Z).$ Alors $K$ possède un $p$-Sylow $\Sigma$, $|K| = p^{\gamma}l$, où $ \text{pgcd}(p,l) = 1$
...
Soit $G \overset{\text{Cayley}}{\hookrightarrow} \mathbb{S}_n \overset{\phi}{\hookrightarrow} GL_n \left( \Z / p\Z \right) $ tel que pour $\sigma \in\mathbb{S}_n $, on a $\phi (\sigma)=Mat(u_\sigma)$. Considérons $(e_1, ...e_n) $ la base canonique de $\left( \Z / p\Z \right) ^n$, soit $\sigma \in \mathbb{S}_n$, on pose : 
