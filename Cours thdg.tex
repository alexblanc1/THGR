...
% l'indentation des items, ainsi que l'espace entre les items est mal définie pour les deux environnements suivants :
\newlist{itemizel}{itemize*}{1} % comme itemize mais sans saut de ligne avant le premier item, ni après le dernier, de profondeur max 1 (pas de itemizel dans un itemizel), et de label par défaut bullet
\setlist[itemizel]{itemjoin=\\, label=$\bullet$}

\newlist{enumeratel}{enumerate*}{1} % comme enumerate mais sans saut de ligne avant le premier item, ni après le dernier, de profondeur max 1 (pas de itemizel dans un itemizel)
\setlist[enumeratel]{itemjoin=\\, label=\arabic*.}
...
\begin{preuve}
\begin{enumerate}
\item $\mathcal{A}_3 = <(1,2,3)>$ donc $\mathcal{A}_3\simeq \Z/3\Z$ est abélien
