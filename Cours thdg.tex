...
$$i) N \triangleleft G \\
ii) G = N H
iii) N \cap H \mathrm{est \; trivial}
$$
\end{definition}
...
\underline{Rq :} On dit alors que $H$ est un complément de $N$ dans $G$. (il n'est en général pas unique : c'est l'analogue des supplémentaires en algèbre linéaire) \\
Par ailleurs, un sous-groupe distingué $N$ n'admet pas nécessairement de complément.

\underline{Rq :} Supposons $i), ii)$ et $iii)$ vérifiés pour $N, H$ et $G$.
...
Soient $N, H$ deux groupes. Soit $\phi : H \rightarrow \mathrm{Aut}(N)$ un morphisme.
...
\begin{prop} L'ensemble $N \times H$ munit de la loi $(x, h) \cdot_{\phi} (y, k) = (x \phi_h(y), hk)$ est un groupe, appelé le produit semi-direct externe de $N$ par $H$ relativement à $\phi$.
On le note $N \rtimes_{\phi} H$.
\end{prop}


\begin{preuve} Il faut vérifier l'associativité, l'existence d'un élèment neutre (qui se trouve être $(1_N, 1_H)$ et l'existence d'un inverse pour chaque élèment : $(x, h)ˆ{-1} = (\phi_{hˆ{-1}} (xˆ{-1}), hˆ{-1})
\end{preuve}

\begin{prop}
    Soit $G$ le produit semi-direct externe de $N$ par $H$, relatif à $\fctsarg{\phi}{H}{\textrm{Aut}(N)}$. On a donc : $G = N\rtimes_{\phi} H$.On note $\ast_{\phi}$ la loi de groupe.\\
    
    Soit $\fonction{I_N}{N}{G}{x}{(x,e_H)}$,\\
    $\fonction{I_H}{H}{G}{h}{(e_N,h)}$\\

    Soit $N' = \text{Im}(I_N)$, c'est un sous-groupe de $G$ iso à $N$, et $H' = \text{Im}(I_H)$, c'est un sous-rgoupe de $G$ iso à $H$.\\

    Alors $G$ est produit semi-direct interne de $N'$ par $H'$. De plus, 
    $I_N(\phi_h(x)) = I_H(h) \ast\phi I_N(x) \ast_{\phi} (I_H(h))^{-1}$
\end{prop}

\remarque{}
Si on note avec les images de $I_N$ et $I_H$ (les réalisations $N'$ de $N$ dans $G$, et $H'$ de $H$ dans $G$)\\
$(\phi_h(x))' = h' \ast_{\phi} x' \ast_{\phi} (h')^{-1}$\\
...
\newpage 
    \textbf{1) $G$ possède un $p$-Sylow}. 
        ...
        (i) Soit $G \overset{\text{Cayley}}{\hookrightarrow} \mathbb{S}_n \overset{\phi}{\hookrightarrow} GL_n \left( \Z / p\Z \right) $ tel que pour $\sigma \in\mathbb{S}_n $, on a $\phi (\sigma)=Mat(u_\sigma)$. Considérons $(e_1, ...e_n) $ la base canonique de $\left( \Z / p\Z \right) ^n$, soit $\sigma \in \mathbb{S}_n$, on pose : 
