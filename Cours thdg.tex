...
Soit $G_{S}$ l'ensemble des sous groupes de G qui contiennent S. \\\\On appelle sous groupe engendré par S l'ensemble : $ \dsp\langle S\rangle := \bigcap _{H\in G_{S}}H$\\\\
...
\begin{preuve}
	...
	$f \text{ est surjective} \\ \iff | Im(f) | =mn \\ \iff |\mathbb{Z}/ker(f)| = mn \; \textit{(grâce au théorème d'isomorphisme)} \\ \iff ker(f)= mn\mathbb{Z} \\ \iff m\mathbb{Z} \cap n\mathbb{Z} = mn\mathbb{Z}\\  \iff m \wedge n =1$
\end{preuve}
...
\begin{theoreme}
	Soit $n=p_1^{\alpha_1} \cdots p_k^{\alpha_k}$, décomposé en facteur premiers. Alors : $$\varphi (n) = n \left (1-\frac{1}{p_1} \right ) \cdots  \left (1-\frac{1}{p_k} \right ) $$
\end{theoreme}
...
On note $\sn$  l'ensemble des permutations de $\{1, 2, ..., n\}$ que l'on munit de la loi de composition : c'est un groupe.
...
Montrer que :
\begin{itemize}[label=$\bullet$]
\item $\text{support}(\sigma^{-1})=text{support}(\sigma)$
\item $\text{support}(\sigma^{k})=\\begin{text{support}\sigma}$
\item deux permutations dont les supports sont disjoints commutent
\end{itemize}
\end{exo}

\begin{definition}{Orbite}
Soit $\sigma\in\sn$. On définit la relation d'équivalence sur $\unn$ : \[i\mathcal{R}j \Leftrightarrow \exists r\in\Z \;|\;\sigma^r(i)=j.\]
La classe de $i$ est notée $\omega(i) = \{\sigma^r(i), r\in\Z\}$ est appelée $\sigma$-orbite de $i$.
\end{definition}

\subsection{Notion de cycle}
\begin{definition}{r-ccle}
$\sigma \in\sn$ est un r-cycle si il existe $j_1, ..., j_r$ dans $\unn$ tq $\sigma(j_1)=j_2, ..., $. Alors supp(sigma)=j1, ...jr
On notera $\sigma = (j_1, ..., j_r)$
\end{definition}
...
\begin{theoreme}
Toute permutation $\sigma\in\sn\setminus\{e\}$ se décompose sous la forme $\sigma=\gamma_1\circ\gamma_2\circ ... \circ\gamma_s$ où $s\in\N^*$, et où les $\gamma_i$ sont des cycles dont les supports sont disjoints deux à deux et $\gamma_i\neq e$. Cette décomposition est unique à l'ordre près des facteurs.
\end{theoreme}

\begin{preuve}
cf livres classiques
\end{preuve}

\begin{exo}
\begin{enumerate}
\item l'ordre de $\sigma$ est égal au ppcm des longueurs des cycles $\gamma_1, ..., \gamma_s$.
...
\item $(1, i)(1,j)(1,i)=(,i)(1,j)(1,i)^{-1}=(i,j)$
\end{enumerate}
...
\underline{\textbf{Cas particulier :}} $(a,b,c)=(a,b)(b,c)$
...
\item $\sigma=(1,2)$ : ici il y a $n-2$ éléments fixés qui donnent chacun une orbite, et $\{1,2\}$ est une autre orbite, d'où $\sign=-1$.
...
\begin{theoreme}
Si $n\geq2$, $\epsilon\longrightarrow\{1,-1\}$ est un morphisme de groupes surjectif.
\end{theoreme}
\begin{preuve}
...
\end{definition}
\remarque{C'est un sous groupe distingué de $s_n$ d'indice 2, car le noyau d'un morphisme}


\remarque{Si $\tau$ est une transposition, $\tau\an\cap\an=\emptyset$, d'où $\sn=\an\sqcup\tau\an$.}
\begin{theoreme}
\begin{enumerate}
...
\item Soient $(i,j,k),(i',j',k')$ deux 3-cycles. Il existe $\sigma\in\sn$ tel que $\sigma(i)=i', \\sigma(j)=j', \sigma(k)=k'$. Alors $\sigma(i,j,k)\sigma^{-1}=(i',j',k')$. Sans perte de généralité, on peut supposer que $\sigma\in\an$, en effet $n\geq5$, donc il existe une transposition $\tau=(r,s)$ avec $r,s\notin\{i,j,k\}$, et on peut remplacer $\sigma$ par $\sigma\tau$.
...
\underline{\textbf{Cas particuliers :}}
\begin{enumerate}
\item $\mathcal{A}_3 = <(1,2,3)>$ donc $\mathcal{A}_3\simeq \Z/3\Z$ est abélien, donc $D(\mathcal{A}_3)={e}$
